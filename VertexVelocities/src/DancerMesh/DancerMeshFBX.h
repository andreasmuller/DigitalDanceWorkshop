...
class DancerMeshFBX : public DancerMesh
{
	...
	//		currentAnimation.setFramerate(120.0f);
		...
		void updateTriangleMesh( ofMesh& _mesh )
		{
			...
			if( _mesh.getNumVertices() == prevTriangleMesh.getNumVertices() )
			{
//				calculateVertexVelocities( ofMesh& _currMesh, ofMesh& _prevMesh )
				//currVertexVelocities
			}
			ofSeedRandom();
		}
		...
		// ---------------------------------------------
		void calculateVertexVelocities( ofMesh& _currMesh, ofMesh& _prevMesh )
		{
		}
